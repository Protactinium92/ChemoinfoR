...
```{r eval =FALSE}
x <- c("readr","tibble","tidyr","ggplot2","dplyr", "factoextra", "FactoMineR",
       "rgl", "colorspace","ggthemes","ggpubr", "readxl")
...
-   **\>3** ==\> TThe data distribution is eptokurtic, which means it tends to produce more outliers than the normal distribution
...
-   The exact test of Fisher is used for sa contingent tables cf. $\chi^{2}$ test
...
```
Here the p-value is under 0.05 for both spplementation and the dosage of vitamin C. That means that the H0 is rejected. This two factors have a significant impact. In the same time, the impact of the interaction between this two factor is also significant (p-value <0.05).

### Graph analysis of the ToothGrowth with ggarrange()

```{r echo=FALSE}
# remove the message error summarise()` has grouped output by 'dose'. You can override using the .groups` argument.
options(dplyr.summarise.inform = FALSE) 
```

```{r}
# Setp 1 cheration of the theme
theme_set(
  theme_classic() +
...
```
It could be interesting to see the sree plot
...
With the code below and a GIMP software, it is possible to create an animation of the 3D picture.

```{r eval=FALSE}
setwd("C:/Users/hpier/Documents/code/R/ChemoinfoR_tuto")
for (i in 1:360) {
  par3d(i, 20) ;
