...
ggplot(iris, aes(x= Sepal.Length))+
  ...
  geom_density(aes(color = Species), size = 1)+ #show the density of each disribution
...
The normal test is sensitive to the sample length. Small effective usually pass the test. That why the visual inspection AND the normal test must to be performed.
...
```
For the two first p-value > 0.05, the test is not significant ==> H0 validate .
The last one, p-value < 0.05 ==> a does not follow this particular law.
...
This test is performed to compare the equality of two means. The H0 hypothesis is that $µ_{1}=µ_{2}$.
