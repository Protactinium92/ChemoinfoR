...
In this case, measurements have been performed independently of the samples. So the first table does not work. So here it will be better that each value receive tag corresponding to its origin (method1 / method2). The code below propose this kind of table:
...
The Binary law can be positive or negative, but in the general cases, it is only the positie which is used. This is the law which is followed by a randomization selection when there are **ONLY TWO** possibility of results: A or B, alive or died. The probability to obtained each evenment must be constant (but not necessary egal). The probability to obtain the A event will be  $p$ and B will be $q=(1-p)$. So, with $n$ selection, the probability to obtain the A event $P(X=k)$ will follow the binomial law $B(n,p)$. The mean $\mu$ correspond to the esperance (the infinity selection) and wil be give by $\mu=np$.
